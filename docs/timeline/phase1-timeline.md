# Phase 1: Foundation - Timeline
**Duration:** Oct 24, 2025 - Feb 28, 2026 (127 days)  
**Work Schedule:** 1 hour/day average  
**Total Hours Available:** 127 hours
---
## Phase 1a: Genesis - Data Structures and Algorithms (Oct 24 - Nov 11, 2025)
**Total Estimated Time:** ~30-35 hours  
**Available Time:** 37 days × 1 hour/day = 37 hours
| Status | Project | Estimated Hours | Timeline | Cumulative Hours |
|--------|---------|----------------|----------|------------------|
| [x] | Dynamic Array (Vector) | 2 | Oct 24-25, 2025 | 2 |
| [x] | Singly LinkedList | 2 | Oct 26-27, 2025 | 4 |
| [x] | Doubly LinkedList | 3 | Oct 28-30, 2025 | 7 |
| [x] | Stack | 2 | Oct 31 - Nov 1, 2025 | 9 |
| [x] | Queue | 2-3 | Nov 2-4, 2025 | 11-12 |
| [x] | Hash Table (Separate Chaining) | 4-5 | Nov 5-8, 2025 | 15-17 |
| [x] | Trie (Prefix Tree) | 3-4 | Nov 9-12, 2025 | 18-21 |
| [X] | LRU Cache | 3-4 | Nov 13-16, 2025 | 21-25 |
| [x] | Red-Black Tree ⭐ Capstone | 12-15 | Nov 17-30, 2025 | 33-40 |
**Progress:** 9/9 complete (100%)
---
## Phase 1b: Evolution - Networking Primitives & Advanced Algorithms (Nov 12, 2025 - Jan 10, 2026)
**Total Estimated Time:** ~38-50 hours  
**Available Time:** 60 days × 1 hour/day = 60 hours
| Status | Project | Estimated Hours | Timeline | Cumulative Hours |
|--------|---------|----------------|----------|------------------|
| [X] | Bloom Filter | 3-4 | Nov 12-15, 2025 | 36-44 |
| [X] | Ring Buffer (Circular Buffer) | 3-4 | Nov 16-19, 2025 | 39-48 |
| [x] | Skip List | 4-5 | Nov 20-24, 2025 | 43-53 |
| [X] | CRC32 / Internet Checksum | 2-3 | Nov 25-27, 2025 | 45-56 |
| [x] | Token Bucket Algorithm | 3-4 | Nov 28 - Dec 1, 2025 | 48-60 |
| [x] | Leaky Bucket Algorithm | 3-4 | Dec 2-5, 2025 | 51-64 |
| [x ] | Exponential Backoff | 2-3 | Dec 6-8, 2025 | 53-67 |
| [ ] | Bellman-Ford Algorithm | 5-6 | Dec 14-19, 2025 | 62-78 |
| [ ] | Sliding Window Protocol ⭐ Capstone | 8-10 | Dec 20, 2025 - Jan 8, 2026 | 70-88 |
**Progress:** 6/10 complete (60%)
---
## Phase 1 Total Progress
**Completed:** 14/19 projects  
**Total Estimated Time:** ~68-85 hours  
**Available Time:** 127 hours
---
## Notes
- All time estimates include project setup (`.h`, `.cpp`, `gtest.cpp`, `makefile`), comprehensive unit testing (GoogleTest), and header file documentation per Google C++ Style Guide
- Phase 1a focuses on C++ implementation mastery and foundational data structures with emphasis on manual memory management
- Phase 1b focuses on networking primitives that directly prepare for Phase 2
- Skipping Min Heap, Dijkstra's, and Union-Find - Min Heap and Dijkstra's were covered in CSE 332, and Union-Find doesn't provide additional C++ memory management practice
- Skipping Consistent Hashing and Radix Tree - Consistent Hashing will be covered in CSE 422 (Advanced Toolkit for Modern Algorithms) HW1
- CSE 422 will cover modern algorithmic techniques like sketching algorithms (count-min sketch), similarity search, dimension reduction, PCA, SVD, spectral graph theory, and MCMC - complementing this low-level systems focus
- Later phases will rely on STL/Boost/absl containers (not these custom ones); the Phase 1 implementations stay as reference material only
- Red-Black Tree and Sliding Window Protocol are the capstone projects for each sub-phase
---
**Legend:**
- [x] Complete
- [ ] Pending
---
**Links:**
- [Phase 1 Overview](../overview/phase1-overview.md)
- [Phase 1a: Genesis Detailed Timeline](genesis-a-timeline.md)
- [Phase 1b: Evolution Detailed Timeline](genesis-b-timeline.md)
- [Back to README](../../README.md)
