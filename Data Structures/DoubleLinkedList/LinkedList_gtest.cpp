...
TEST(LinkedListTest, find) {
  LinkedList<int> list;
  ASSERT_EQ(list.find(0), list.end());  // Not found returns end()

  for (int i = 0; i < 3; i++) {
    list.push_back(i);
  }

  auto node1 = list.find(0);
  auto node2 = list.find(1);
  auto node3 = list.find(2);

  ASSERT_NE(node1, list.end());  // Check found
  ASSERT_NE(node2, list.end());
  ASSERT_NE(node3, list.end());

  ASSERT_EQ(*node1, 0);  // Use * to get value
  ASSERT_EQ(*node2, 1);
  ASSERT_EQ(*node3, 2);

  // Remove the node1->next tests - that's internal implementation
}

TEST(LinkedListTest, FindConst) {
  LinkedList<int> temp;
  const LinkedList<int> emptyTest = temp;
  ASSERT_EQ(emptyTest.find(0), emptyTest.end());  // Not nullptr
  ...
  auto node1 = list.find(0);  // ConstIterator
  auto node2 = list.find(1);
  auto node3 = list.find(2);

  ASSERT_NE(node1, list.end());
  ASSERT_NE(node2, list.end());
  ASSERT_NE(node3, list.end());
