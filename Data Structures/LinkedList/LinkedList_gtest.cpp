...
TEST(LinkedListTest, find) {
  LinkedList<int> list;
  ASSERT_EQ(list.find(0), list.end());  // Change: nullptr → list.end()
  for (int i = 0; i < 3; i++) {
    list.push_back(i);
  }
  auto node1 = list.find(0);  // Returns Iterator now
  auto node2 = list.find(1);
  auto node3 = list.find(2);

  ASSERT_NE(node1, list.end());  // Check it was found
  ASSERT_NE(node2, list.end());
  ASSERT_NE(node3, list.end());

  ASSERT_EQ(*node1, 0);  // Change: node1->val → *node1
  ASSERT_EQ(*node2, 1);
  ASSERT_EQ(*node3, 2);
}
...
TEST(LinkedListTest, FindConst) {
  ...
  ASSERT_EQ(emptyTest.find(0),
            emptyTest.cend());  // Change: nullptr → emptyTest.end()
  for (int i = 0; i < 3; i++) {
    temp.push_back(i);
  }
  const LinkedList<int> list = temp;

  auto node1 = list.find(0);  // Returns ConstIterator now
  auto node2 = list.find(1);
  auto node3 = list.find(2);

  ASSERT_NE(node1, list.cend());
  ASSERT_NE(node2, list.cend());
  ASSERT_NE(node3, list.cend());
