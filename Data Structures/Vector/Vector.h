...
class Vector {
 ...
 private:
  ...
  /**
   * Cleans up each index of the array
   *
   * ARGS:
   * alloactor: the alloactor stl which will be used to deallocate memory
   */
  void clean_up(std::allocator<T> allocator);
};
...
template <typename T>
Vector<T>::Vector() : capacity_(initial_capacity), size_(0) {
  std::allocator<T> allocator;
  array_ = allocator.allocate(initial_capacity);
}

template <typename T>
Vector<T>::Vector(const Vector& other)
    : capacity_(other.capacity_), size_(other.size_) {
  std::allocator<T> alloactor;
  array_ = alloactor(capacity_);
...
Vector<T>& Vector<T>::operator=(const Vector& other) {
  ...
  std::allocator<T> allocator;
  // shared pointer in case of an error during allocation or copy
  std::unique_ptr<T*> new_array = allocator.allocate(new_capacity);
...
template <typename T>
void Vector<T>::clean_up(std::allocator<T> allocator) {
  for (size_type i = 0; i < size_; i++) {
    array_[i].~T();
  }
